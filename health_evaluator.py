# health_evaluator.py
"""
电梯健康度评估模块 (V2.4.2 修正版)
- 修正：曳引机轴承组层级结构，bearing_fault包含子项bearing_inner/outer/cage/ball
- 保留轿厢Z轴/XY轴分离、平滑短板惩罚、模糊等级、工程化测试脚本
"""
...
GLOBAL_SETTINGS = {
    ...
    "env_sensors": {
        "temp_t1": 20.0, "temp_t2": 60.0,
        "disp_t1": 5.0, "disp_t2": 10.0,
        "noise_threshold": 1.0, "noise_slope": 15.0
    },
...
# --------------------------- 环境与电气评估函数 ---------------------------
def evaluate_env_temperature(temp): return linear_score(temp, GLOBAL_SETTINGS["env_sensors"]["temp_t1"], GLOBAL_SETTINGS["env_sensors"]["temp_t2"])
def evaluate_env_water(water): return 0.0 if water == 1 else 100.0
def evaluate_env_current(current): return 0.0 if current == 1 else 100.0
def evaluate_env_displacement(disp): return linear_score(disp, GLOBAL_SETTINGS["env_sensors"]["disp_t1"], GLOBAL_SETTINGS["env_sensors"]["disp_t2"])
def evaluate_env_noise(ratio):
    th, slope = GLOBAL_SETTINGS["env_sensors"]["noise_threshold"], GLOBAL_SETTINGS["env_sensors"]["noise_slope"]
...
def evaluate_traction(data: Dict, config: Dict) -> Tuple[float, Dict[str, Union[float, Dict]]]:
    ...
    # 综合曳引机分数 (加权后短板平滑)
    # 直接计算加权平均，注意轴承组取它的score
    fault_scores = {
        "motor_fault": details["motor_fault"],
        "bearing_fault": details["bearing_fault"]["score"],  # 取出综合分
...
class HealthEvaluator:
    ...
    def evaluate(self, data: Dict) -> Dict:
        ...
        for device, weight in self.config["device_weights"].items():
            ...
            # 格式化 details，处理嵌套结构（如 bearing_fault）
            formatted_details = {}
        ...
        env_map = {
            "enable_temperature": ("temperature", 20.0, evaluate_env_temperature),
            "enable_water": ("water", 0, evaluate_env_water),
            "enable_current": ("motor_current", 0, evaluate_env_current),
            "enable_noise": ("noise_ratio", 1.0, evaluate_env_noise),
            "enable_displacement": ("displacement", 0.0, evaluate_env_displacement)
        }
        
        for key, (field, default_val, e_func) in env_map.items():
            if switches.get(key):
                s = e_func(env_data.get(field, default_val))
                env_scores[field] = self._format_score_node(s)
